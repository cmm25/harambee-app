  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      <div className="lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
          <input type='text' placeholder='Search for Harambees' className='flex w-full font-epilogue font-normal text-[14px] 
          placeholder:text-[#4b5264] text-white bg-transparent outline-none'/>
          <div className='w-[72px] h-full rounded-[20px] bg-[#A5582C] flex justify-center items-center cursor-pointer'>
      ...
      <div className='sm:flex-hidden flex-row justify-end gap-4'>
        <CustomButton
          btnType="button"
          title={address? 'Make a Harambee': 'Connect'}
          styles ={address?'bg-[#1dcc071]':'bg-[#A5582C]'}
          handleClick ={()=>{
            if(address){ navigate ('create-harambee')} else {connect()}
          }}
              
        />
        <Link to= "/profile">
          <div className='w-[52px] h-[52px] rounded-full flex justify-center items-center cursor-pointer'>
            {/* <img src={logo} alt='user' className='w-[60%] object-contain 5-[60%]'/> */}
          </div>
        </Link>
      </div>
      {/*navigation support for small screens*/}
      <div className='sm:hidden flex justify-between items-center relative'>
          <div className='w-[40px] h-[40px] rounded-[10px] flex justify-center items-center cursor-pointer'>
            <img src={thirdweb} alt='user' className='w-[60%] object-contain 5-[60%]'/>
          </div>  
          ...
          ${!toggleDrawer?'-translate-y-[100vh]': 'translate-y-0'} transition-all duration-700`}>
            ...
            <div className='flex mx-4'>
              <CustomButton
                btnType="button"
                title={address? 'Make a Harambee': 'Connect'}
                styles ={address?'bg-[#452212]':'bg-[#8c6dfd]'}
                handleClick ={()=>{
                  if(address){ navigate ('create-harambee')} else {connect()}
                }}
              />
